...
def calculate_histogram_limits(data: np.ndarray) -> tuple:
    ...
    if _fraction_high < 1:
        ...
        _counts_fine, _edges_fine = np.histogram(
            data, bins=32768, range=(_edges[0], _edges[_index_stop])
        )
        _cumcounts_fine = np.cumsum(_counts_fine / data.size)
        _index_stop2 = max(1, np.where(_cumcounts_fine <= _fraction_high)[0].size)
        _cmap_limit_high = _edges_fine[_index_stop2]
