...
class CompositeCreator_FrameBuilder(CreateWidgetsMixIn, ParameterWidgetsMixIn,
    ...
    def build_frame(self):
        ...
        self.create_progress_bar(
            'progress', parent_widget=self._widgets['config'],
            gridPos=(-1, 0, 1, 2), visible=False,
            fixedWidth=CONFIG_WIDGET_WIDTH, minimum=0, maximum=100)

        self.create_button(
            'but_abort', 'Abort composite creation', gridPos=(-1, 0, 1, 2),
            parent_widget=self._widgets['config'], enabled=True, visible=False,
            fixedWidth=CONFIG_WIDGET_WIDTH)

        self.create_button(
            'but_show', 'Show composite', gridPos=(-1, 0, 1, 2),
            parent_widget=self._widgets['config'], enabled=False,
            fixedWidth=CONFIG_WIDGET_WIDTH)
        ...
        self.create_spacer(
            'spacer_bottom', parent_widget=self._widgets['config'],
