...
__all__ = ["Scan", "SCAN_DEFAULT_PARAMS", "SCAN_LEGACY_PARAMS"]
...
class Scan(ObjectWithParameterCollection):
    ...
    @property
    def n_frames_required(self) -> int:
        """
        Get the total number of frames/spectra required to process the full Scan.

        Returns
        -------
        int
            The total number of frames/spectra to be processed.
        """
        return self.get_param_value("frame_indices_per_scan_point") * (
            self.n_points - 1
        ) + self.get_param_value("scan_frames_per_point")
