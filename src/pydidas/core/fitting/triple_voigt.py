...
class TripleVoigt(Voigt):
    ...
    name = "Triple Voigt"
    num_peaks = 3
    param_bounds_low: ClassVar[list[float]] = Voigt.param_bounds_low * num_peaks
    param_bounds_high: ClassVar[list[float]] = Voigt.param_bounds_high * num_peaks
    param_labels: ClassVar[list[str]] = [
        f"{key}{i}" for i in range(num_peaks) for key in Voigt.param_labels
    ]
