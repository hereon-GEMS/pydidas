    ...
    if _nxdata_signal_only and item.attrs.get("NX_class", "") == "NXdata":
        ...
        for _key in item:
            ...
            if file_ref == _item.file:
                ...
                # external datasets' .name refers to the .name in the external
                # data file, not the link in the current file and must be replaced
                # by the local name to reference the dataset in the current file
                # correctly for opening it.
