...
class QuickIntegrationFrame(BaseFrame):
    ...
    def _copy_diffraction_exp(self) -> None:
        ...
        if _det_name not in PYFAI_DETECTOR_MODELS_OF_SHAPES[self._EXP.det_shape]:
            _matches = [
                _name
                for _name in PYFAI_DETECTOR_MODELS_OF_SHAPES[self._EXP.det_shape]
                if _det_name in _name
            ]
