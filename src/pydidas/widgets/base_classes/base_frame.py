# This file is part of pydidas.
#
# Copyright 2023 - 2026, Helmholtz-Zentrum Hereon
# SPDX-License-Identifier: GPL-3.0-only
#
# pydidas is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License version 3 as
# published by the Free Software Foundation.
#
# Pydidas is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with Pydidas. If not, see <http://www.gnu.org/licenses/>.

"""
Module with the BaseFrame, the widget from which all main pydidas frames
should inherit.
"""

__author__ = "Malte Storm"
__copyright__ = "Copyright 2023 - 2026, Helmholtz-Zentrum Hereon"
__license__ = "GPL-3.0-only"
__maintainer__ = "Malte Storm"
__status__ = "Production"
__all__ = ["BaseFrame"]


from typing import Any, ClassVar

from qtpy import QtCore, QtGui, QtWidgets

from pydidas.core import (
    ParameterCollection,
    PydidasQsettingsMixin,
    UserConfigError,
)
from pydidas.core.utils import ShowBusyMouse
from pydidas.resources import icons
from pydidas.widgets.base_classes.parameter_widget_mixin import ParameterWidgetMixIn
from pydidas.widgets.base_classes.widget_factory_mixin import WidgetFactoryMixIn


class BaseFrame(
    PydidasQsettingsMixin,
    WidgetFactoryMixIn,
    ParameterWidgetMixIn,
    QtWidgets.QWidget,
):
    """
    The BaseFrame is a subclassed QWidget and the base class for all
    Frames in pydidas.

    The BaseFrame is instantiated lazily and the __init__ does not
    create all subwidgets. Instead, the following three methods are
    provided:

    - build_frame(): Build all widgets of the frame.
    - connect_signals(): Connect all the required signals for the frame.
    - finalize_ui(): Finalize the UI initialization.

    The frame_activated slot is called when the frame is activated
    by the central widget. If the frame has not yet been built, it will
    finish the initialization by calling the three methods above.

    The BaseFrame also provides a mechanism to export and restore the
    state of the frame, including parameter values and widget visibility.
    The export_state() method returns a dictionary with the frame's state
    information, while the restore_state() method takes a dictionary with
    the state information and restores the frame's state accordingly.
    """

    show_frame = True
    menu_icon = "qt-std::SP_TitleBarContextHelpButton"
    menu_title = ""
    menu_entry = ""
    params_not_to_restore: ClassVar[list[str]] = []
    status_msg = QtCore.Signal(str)
    sig_closed = QtCore.Signal()
    sig_this_frame_activated = QtCore.Signal()
    default_params = ParameterCollection()

    def __init__(self, **kwargs: Any) -> None:
        """
        Initialize the BaseFrame.

        Parameters
        ----------
        **kwargs : Any
            Any additional keyword arguments which might be used by subclasses.
        """
        QtWidgets.QWidget.__init__(self, kwargs.get("parent", None))
        self.setWindowIcon(icons.pydidas_icon_with_bg())
        self.setVisible(False)
        self.setUpdatesEnabled(False)
        WidgetFactoryMixIn.__init__(self)
        PydidasQsettingsMixin.__init__(self)
        ParameterWidgetMixIn.__init__(self)

        _layout = QtWidgets.QGridLayout()
        _layout.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
        self.setLayout(_layout)
        self.frame_index = -1
        self.menu_entry = kwargs.get("menu_entry", self.menu_entry)
        self.menu_title = kwargs.get("title", self.menu_title)
        self.icon = kwargs.get("icon", self.menu_icon)
        self._config: dict[str, Any] = {"built": False}

    @QtCore.Slot(int)
    def frame_activated(self, index: int) -> None:
        """
        Received signal that frame has been activated.

        This method is called when this frame becomes activated by the
        central widget. By default, this method will perform no actions.
        If specific frames require any actions, they will need to overwrite
        this method.

        Parameters
        ----------
        index : int
            The index of the activated frame.
        """
        if index == self.frame_index and not self._config["built"]:
            with ShowBusyMouse():
                self.build_frame()
                self.setUpdatesEnabled(True)
                self.connect_signals()
                self.finalize_ui()
                self._config["built"] = True
                try:
                    _state = self._config.pop("state", None)
                    if _state:
                        self.restore_state(_state)
                except UserConfigError as exc:
                    raise UserConfigError(
                        f"- Error restoring state for frame {self.menu_title}: {exc}\n"
                    )
        self._config["frame_active"] = index == self.frame_index
        if index == self.frame_index:
            self.sig_this_frame_activated.emit()  # type: ignore[attr-defined]

    def build_frame(self) -> None:
        """Build all widgets of the frame."""

    def connect_signals(self) -> None:
        """Connect all the required signals for the frame."""

    def finalize_ui(self) -> None:
        """finalize the UI initialization."""

    def set_status(self, text: str) -> None:
        """
        Emit a status message to the main window.

        Parameters
        ----------
        text : str
            The status message to be emitted.
        """
        self.status_msg.emit(text)  # type: ignore[attr-defined]

    def export_state(self):
        """
        Export the state of the Frame for saving.

        Returns
        -------
        frame_index : int
            The frame index which can be used as key for referencing the state.
        information : dict
            A dictionary with all the information required to export the
            frame's state.
        """
        _all_params = self.get_param_values_as_dict(filter_types_for_export=True)
        _params = {
            _key: _value
            for _key, _value in _all_params.items()
            if _key not in self.params_not_to_restore
        }
        return self.menu_entry, {
            "frame_index": self.frame_index,
            "params": _params,
            "menu_entry": self.menu_entry,
            "class": self.__class__.__name__,
        }

    def inject_frame_state(self, state: dict) -> None:
        """
        Inject a previously stored frame state and save it for restoration.

        Parameters
        ----------
        state : dict
            A dictionary with the frame state information. The exact contents
            may vary for each BaseFrame implementation.
        """
        self._config["state"] = state  # type: ignore[arg-type]
        self.frame_index = state["frame_index"]
        for _key, _val in state["params"].items():
            if _key in self.params and _key not in self.params_not_to_restore:
                try:
                    self.set_param_value(_key, _val)
                except (ValueError, TypeError):
                    raise UserConfigError(f"- Error restoring parameter '{_key}'\n")

    def restore_state(self, state: dict) -> None:
        """
        Restore the frame's state from stored information.

        The default implementation in the BaseFrame will update all Parameters
        (and associated widgets) and restore the visibility of all widgets. If the
        frame has not yet been built, the state information is only stored internally
        and not yet applied. It will be applied at the next frame activation.

        Parameters
        ----------
        state : dict
            A dictionary with 'params' and 'visibility' keys and the respective
            information for both.
        """
        for _key, _val in state["params"].items():
            if _key in self.param_widgets and _key not in self.params_not_to_restore:
                self.update_param_widget_value(_key, _val)

    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        """
        Reimplement the closeEvent to emit a signal about the closing.

        Parameters
        ----------
        event : QtGui.QCloseEvent
            The event which triggered the closeEvent.
        """
        self.sig_closed.emit()
        super().closeEvent(event)
