...
from pydidas.widgets.dialogs.warning_box import WarningBox
from pydidas.widgets.utilities import get_pyqt_icon_from_str


class PydidasFrameStack(QtWidgets.QStackedWidget):
    """
    ...
    def register_frame(self, frame: BaseFrame) -> None:
        """
        Register a Frame with the stacked widget.

        This method will register a Frame and hold a reference to the
        frame index by the Frame's menu_entry.
        ...
        index = QtWidgets.QStackedWidget.addWidget(self, frame)  # type: ignore[arg-type]
    ...
    def activate_widget_by_name(self, ref_name: str) -> None:
        """
    ...
    def remove_widget_by_name(self, ref_name: str) -> None:
        """
        Remove a widget by its reference name.

        This method finds the widget associated with the reference name
        and deletes it from the QStackedWidget.

        Note: This does not delete the widget itself, only the reference
    ...
    def addWidget(
        self, widget: QtWidgets.QWidget | None = None, name: str | None = None
    ) -> None:
        """
    ...
    def removeWidget(self, frame: BaseFrame) -> None:
        """
        Remove a frame from the QStackedWidget.

        This overloaded method removes a frame widget from the QStackedWidget
        and also dereferences it from the metadata.
    ...
    def reset(self) -> None:
        ...
        while len(self._frames) > 0:
            self.removeWidget(self.widget(0))  # type: ignore[arg-type]
    ...
    def restore_frame_state_info(self, state: dict[str, dict]) -> None:
        ...
        state : dict[str, dict]
            ...
            The keys are labeled based on the menu entries and the values
            are dictionaries with the state information for each frame.
        """
        ...
        if _unrestored_frames:
            WarningBox(
                "Unrestored frames",
                (
                    "No stored state information was found for the "
        ...
        if _missing_frames:
            WarningBox(
                ...
                (
                    "The following frames had a stored state but were not "
                    "found in the current GUI configuration:\n"
