...
"""
Module with the _DirectoryExplorerFilterModel widget, which is used to filter
the file system.
"""
...
__all__ = ["_DirectoryExplorerFilterModel"]
...
class _DirectoryExplorerFilterModel(QSortFilterProxyModel):
    """
    ...
    def setSourceModel(self, sourceModel: QtCore.QAbstractItemModel) -> None:
        ...
        if not isinstance(sourceModel, QtWidgets.QFileSystemModel):
            raise TypeError(
                "_DirectoryExplorerFilterModel requires a QFileSystemModel."
            )
