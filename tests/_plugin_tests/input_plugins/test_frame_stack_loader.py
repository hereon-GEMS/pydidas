...
class TestFrameStackLoader(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_creation(self):
        plugin = PLUGIN_COLLECTION.get_plugin_by_name("FrameStackLoader")()
    ...
    def test_execute__simple(self):
        plugin = PLUGIN_COLLECTION.get_plugin_by_name("FrameStackLoader")()
        plugin.set_param_value("frame_count", self._frame_count)
        _index = 0
        plugin.pre_execute()
    ...
    def test_execute__frame_count_too_high(self):
        plugin = PLUGIN_COLLECTION.get_plugin_by_name("FrameStackLoader")()
        plugin.set_param_value("frame_count", self._n + 1)
        _index = 0
        plugin.pre_execute()
        with self.assertRaises(UserConfigError):
