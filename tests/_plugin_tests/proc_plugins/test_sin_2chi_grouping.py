...
def test_execute_validation_basic(
    plugin_fixture, base_dataset_factory, unit, expected_unit
):
    ...
    assert np.all(
        result.axis_ranges[1] == np.full((result.shape[1],), np.sin(2 * np.arctan(2)))
    )
...
def test__calculate_sin2_chi_values_invalid_input_type(plugin_fixture, invalid_input):
    with pytest.raises(
        UserConfigError, match="Input must be an instance of np.ndarray."
    ):
...
def test__calculate_sin2_chi_values_invalid_s2c_values(plugin_fixture, invalid_value):
    with pytest.raises(
        UserConfigError, match="Values in s2c_values must be between 0 and 1 inclusive."
    ):
...
@pytest.mark.parametrize(
    "axis_labels, expected_error_message",
    [
        (
...
def test__ensure_axis_labels_invalid_input(
...
@pytest.mark.parametrize(
    "data, axis_labels, axis_ranges, data_unit, expected_error_message",
    [
        (
...
def test_calculate_diff_d_spacing_vs_sin_2chi_invalid_input(
    plugin_fixture,
    create_dataset,
    data,
    axis_labels,
