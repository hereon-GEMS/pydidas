...
class TestRoiSliceManager(unittest.TestCase):
    ...
    def test_modulate_roi_keys__negative_value(self):
        ...
        self.assertEqual(obj.roi[0], _roi[0])
        self.assertEqual(
            obj.roi[1], slice(_roi[1].start + _shape[1], _roi[1].stop + _shape[1])
        )
    ...
    def test_convert_roi_key_to_slice_objects__4_ints(self):
        ...
        self.assertEqual(obj._roi, (slice(_roi[0], _roi[1]), slice(_roi[2], _roi[3])))
    ...
    def test_roi_coords_getter(self):
        ...
        obj = RoiSliceManager(roi=_roi)
        self.assertEqual(
            obj.roi_coords, (_roi[0].start, _roi[0].stop, _roi[1], _roi[2])
        )
    ...
    def test_merge_rois__two_normal_rois_with_ranges(self):
        ...
        _stop2 = (97, 98)
        obj = RoiSliceManager()
        obj.roi = [slice(_start1[0], _stop1[0]), slice(_start1[1], _stop1[1])]
        _roi2 = [slice(_start2[0], _stop2[0]), slice(_start2[1], _stop2[1])]
        obj.apply_second_roi(_roi2)
        for _axis in [0, 1]:
            self.assertEqual(obj.roi[_axis].start, _start1[_axis] + _start2[_axis])
            self.assertEqual(obj.roi[_axis].stop, min(_stop1[_axis], _stop2[_axis]))
    ...
    def test_merge_rois__two_normal_rois_with_ranges_and_offset(self):
        ...
        for _axis in [0, 1]:
            self.assertEqual(obj.roi[_axis].start, _start1[_axis] + _start2[_axis])
            self.assertEqual(
                obj.roi[_axis].stop, min(_stop1[_axis], _start1[_axis] + _stop2[_axis])
            )
    ...
    def test_merge_rois__two_rois_with_negative_stop_and_shape(self):
        ...
        _x2 = (43, -4)
        _shape = (150, 150)
        obj = RoiSliceManager(roi=[slice(*_y1), slice(*_x1)], input_shape=_shape)
        _roi2 = [slice(*_y2), slice(*_x2)]
        _roi2 = _y2 + _x2
        obj.apply_second_roi(_roi2)
    ...
    def test_modulate_roi_keys_1d__negative_value(self):
        ...
        obj._modulate_roi_keys()
        self.assertEqual(
            obj.roi[0], slice(_roi[0].start + _shape[0], _roi[0].stop + _shape[0])
        )
    ...
    def test_merge_rois_1d__two_normal_rois_with_ranges_and_offset(self):
        ...
        obj.apply_second_roi(_roi2)
        self.assertEqual(obj.roi[0].start, _start1[0] + _start1[1])
        self.assertEqual(obj.roi[0].stop, min(_stop1[0], _start1[0] + _stop1[1]))
