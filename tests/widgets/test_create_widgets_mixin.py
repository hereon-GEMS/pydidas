# This file is part of pydidas.
#
# Copyright 2023 - 2026, Helmholtz-Zentrum Hereon
# SPDX-License-Identifier: GPL-3.0-only
#
# pydidas is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License version 3 as
# published by the Free Software Foundation.
#
# Pydidas is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with Pydidas. If not, see <http://www.gnu.org/licenses/>.

"""Unit tests for the CreateWidgetsMixIn class."""

__author__ = "Malte Storm"
__copyright__ = "Copyright 2023 - 2026, Helmholtz-Zentrum Hereon"
__license__ = "GPL-3.0-only"
__maintainer__ = "Malte Storm"
__status__ = "Production"


import random
import string
import unittest
from typing import Any

from qtpy import QtWidgets

from pydidas.core import PydidasGuiError
from pydidas.widgets.factory.create_widgets_mixin import CreateWidgetsMixIn
from pydidas.widgets.utilities import get_grid_pos, get_widget_layout_args


class _TestWidget(QtWidgets.QWidget, CreateWidgetsMixIn):
    def __init__(self, *args: object, parent: object = None, **kwargs: Any) -> None:
        super().__init__(parent)
        CreateWidgetsMixIn.__init__(self)
        self.hash = hash(self)
        self.name = "".join(random.choice(string.ascii_letters) for i in range(20))


class TestCreateWidgetsMixIn(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.widgets = []

    @classmethod
    def tearDownClass(cls) -> None:
        while cls.widgets:
            w = cls.widgets.pop()
            w.deleteLater()

    def setUp(self) -> None: ...

    def tearDown(self) -> None: ...

    def get_widget(
        self, layout: type[QtWidgets.QLayout] | None = QtWidgets.QGridLayout
    ) -> _TestWidget:
        _w = _TestWidget()
        if layout is not None:
            _w.setLayout(layout())  # type: ignore[call-arg]
        self.widgets.append(_w)
        return _w

    def testget_grid_pos_default(self) -> None:
        obj = self.get_widget()
        _grid_pos = get_grid_pos(obj)
        self.assertEqual(_grid_pos, (0, 0, 1, 1))

    def testget_grid_pos_gridPos(self) -> None:
        _gridPos = (2, 7, 5, 3)
        obj = self.get_widget()
        _grid_pos = get_grid_pos(obj, gridPos=_gridPos)  # type: ignore[arg-type]
        self.assertEqual(_grid_pos, _gridPos)

    def testget_grid_pos_kwargs(self) -> None:
        _gridPos = (2, 7, 5, 3)
        obj = self.get_widget()
        _grid_pos = get_grid_pos(
            obj,
            row=_gridPos[0],  # type: ignore[arg-type]
            column=_gridPos[1],  # type: ignore[arg-type]
            n_rows=_gridPos[2],  # type: ignore[arg-type]
            n_columns=_gridPos[3],  # type: ignore[arg-type]
        )
        self.assertEqual(_grid_pos, _gridPos)

    def testget_grid_pos_auto_row(self) -> None:
        _gridPos = (-1, 7, 5, 3)
        obj = self.get_widget()
        _grid_pos = get_grid_pos(obj, gridPos=_gridPos)  # type: ignore[arg-type]
        self.assertEqual(_grid_pos, (0,) + _gridPos[1:])

    def testget_grid_pos_auto_row_more_items(self) -> None:
        _gridPos = (-1, 7, 5, 3)
        _nwidget = 4
        obj = self.get_widget()
        for i in range(_nwidget):
            _w = _TestWidget()
            obj.layout().addWidget(_w, i, 0, 1, 1)  # type: ignore[call-arg]
        _grid_pos = get_grid_pos(obj, gridPos=_gridPos)  # type: ignore[arg-type]
        self.assertEqual(_grid_pos, (_nwidget,) + _gridPos[1:])

    def testget_widget_layout_args_no_layout(self) -> None:
        obj = _TestWidget()
        with self.assertRaises(PydidasGuiError):
            get_widget_layout_args(obj)

    def test_get_widget_layout_box(self) -> None:
        _stretch = 1.3
        _alignment = "random"
        obj = _TestWidget()
        obj.setLayout(QtWidgets.QVBoxLayout())
        items = get_widget_layout_args(
            obj,
            stretch=_stretch,
            alignment=_alignment,  # type: ignore[arg-type]
        )
        self.assertEqual(items, [_stretch, _alignment])

    def test_get_widget_layout_stacked(self) -> None:
        _stretch = 1.3
        _alignment = "random"
        obj = _TestWidget()
        obj.setLayout(QtWidgets.QStackedLayout())
        items = get_widget_layout_args(
            obj,
            stretch=_stretch,
            alignment=_alignment,  # type: ignore[arg-type]
        )
        self.assertEqual(items, [])

    def test_get_widget_layout_grid_no_alignment(self) -> None:
        _gridPos = (2, 7, 5, 3)
        obj = _TestWidget()
        obj.setLayout(QtWidgets.QGridLayout())
        items = get_widget_layout_args(
            obj,
            gridPos=_gridPos,
            alignment=None,  # type: ignore[arg-type]
        )
        self.assertEqual(items, [*_gridPos])

    def test_get_widget_layout_grid(self) -> None:
        _gridPos = (2, 7, 5, 3)
        obj = _TestWidget()
        obj.setLayout(QtWidgets.QGridLayout())
        items = get_widget_layout_args(obj, gridPos=_gridPos)  # type: ignore[arg-type]
        self.assertEqual(items, [*_gridPos])

    def testget_widget_layout_args(self) -> None:
        obj = self.get_widget()
        _grid_pos = get_grid_pos(obj)
        self.assertEqual(_grid_pos, (0, 0, 1, 1))

    def test_init(self) -> None:
        obj = _TestWidget()
        self.assertIsInstance(obj, CreateWidgetsMixIn)
        self.assertTrue(hasattr(obj, "_widgets"))

    def test_create_label(self) -> None:
        obj = self.get_widget()
        obj.create_label("ref", "Test text", parent_widget=obj)
        _label = obj._widgets["ref"]
        self.assertEqual(
            _label,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_label, QtWidgets.QLabel)

    def test_create_line(self) -> None:
        obj = self.get_widget()
        obj.create_line("ref", parent_widget=obj)
        _line = obj._widgets["ref"]
        self.assertEqual(
            _line,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_line, QtWidgets.QFrame)

    def test_create_spacer(self) -> None:
        obj = self.get_widget()
        obj.create_spacer("ref", parent_widget=obj)
        _spacer = obj._widgets["ref"]
        self.assertEqual(
            _spacer,
            obj.layout().itemAtPosition(0, 0),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_spacer, QtWidgets.QSpacerItem)

    def test_create_button(self) -> None:
        _text = "button text"
        obj = self.get_widget()
        obj.create_button("ref", _text, parent_widget=obj)
        _but = obj._widgets["ref"]
        self.assertEqual(
            _but,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_but, QtWidgets.QPushButton)
        self.assertEqual(_but.text(), _text)

    def test_create_spinbox(self) -> None:
        obj = self.get_widget()
        obj.create_spin_box("ref", parent_widget=obj)
        _spin = obj._widgets["ref"]
        self.assertEqual(
            _spin,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_spin, QtWidgets.QSpinBox)

    def test_create_progress_bar(self) -> None:
        obj = self.get_widget()
        obj.create_progress_bar("ref", parent_widget=obj)
        _bar = obj._widgets["ref"]
        self.assertEqual(
            _bar,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_bar, QtWidgets.QProgressBar)

    def test_create_check_box(self) -> None:
        _text = "another test text"
        obj = self.get_widget()
        obj.create_check_box("ref", _text, parent_widget=obj)
        _box = obj._widgets["ref"]
        self.assertEqual(
            _box,
            obj.layout().itemAtPosition(0, 0).widget(),  # type: ignore[union-attr]
        )
        self.assertIsInstance(_box, QtWidgets.QCheckBox)
        self.assertEqual(_box.text(), _text)

    def test_create_widget__with_layout_args(self) -> None:
        obj = self.get_widget()
        obj.create_any_widget("ref", _TestWidget, layout_kwargs={})
        self.assertTrue("ref" in obj._widgets)

    def test_create_any_widget__plain(self) -> None:
        obj = self.get_widget()
        obj.create_any_widget("ref", _TestWidget)
        self.assertTrue("ref" in obj._widgets)

    def test_create_any_widget__wrong_ref(self) -> None:
        obj = self.get_widget()
        with self.assertRaises(TypeError):
            obj.create_any_widget(12, _TestWidget)  # type: ignore[arg-type]

    def test_add_any_widget__wrong_ref(self) -> None:
        obj = self.get_widget()
        with self.assertRaises(TypeError):
            obj.add_any_widget(12, _TestWidget())  # type: ignore[arg-type]

    def test_add_any_widget__plain(self) -> None:
        obj = self.get_widget()
        obj.add_any_widget("ref", _TestWidget(), layout_kwargs={})
        self.assertTrue("ref" in obj._widgets)


if __name__ == "__main__":
    unittest.main()
